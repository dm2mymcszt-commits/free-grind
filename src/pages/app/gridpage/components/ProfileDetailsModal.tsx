...
export function ProfileDetailsModal({
    ...
    useEffect(() => {
        ...
        if (!el || !isOpen || selectedPhotoIndex !== null || variant !== "page" || (!onPrevProfile && !onNextProfile)) {
            return;
        }
    ...
    }, [isOpen, selectedPhotoIndex, variant, onNextProfile, onPrevProfile]);

    useModalClose({ isOpen, onClose });
    const handleBackdropClose = useMemo(() => createBackdropCloseHandler(onClose), [onClose]);

    useEffect(() => {
        if (!isOpen) setSelectedPhotoIndex(null);
    }, [isOpen]);
    ...
    const photoUrls = useMemo(() => {
        return activeProfilePhotoHashes.map((hash) => getProfileImageUrl(hash, "1024x1024"));
    }, [activeProfilePhotoHashes]);

    const renderPhotoExtraInfo = useCallback((index: number) => {
        ...
        return (
            <p className="inline-flex items-center gap-1 rounded-full bg-black/65 px-3 py-1 text-xs font-semibold text-white ring-1 ring-white/25">
                ...
                `}</style>
                {meta.takenOnGrindr ? (
                    <img src={freegrindLogo} alt={t("chat.thread.taken_on_grindr")} className="h-3.5 w-3.5 rounded-full logo-shine" />
                ) : null}
                {meta.createdAt ? <span>{formatDateTime24(meta.createdAt)}</span> : null}
            </p>
        );
    }, [activeProfilePhotoHashes, photoCreatedAtByHash, t]);

    const openPhotoViewer = (index: number) => setSelectedPhotoIndex(index);
                        ...
                        <div className="relative flex w-full items-center gap-3">
                            <div className="pointer-events-auto flex shrink-0 items-center gap-3">
                                <button
                                    type="button"
                                    onClick={onClose}
                                    className={`inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border transition-colors ${headerScrolled ? "border-[var(--border)] bg-[var(--surface-2)] text-[var(--text)]" : "border-white/45 bg-transparent text-white shadow-[0_10px_28px_-18px_rgba(0,0,0,0.95)] backdrop-blur-md"}`}
            ...
            >
                <div className="flex items-center gap-3 border-b border-[var(--border)] bg-[var(--surface-2)] px-4 py-3 sm:px-5">
                    ...
                    <button
                        type="button"
                        onClick={onClose}
                        className="shrink-0 rounded-lg border border-[var(--border)] bg-[var(--surface)] p-2 transition hover:bg-[var(--surface-2)]"
