...
import type { InterestTab, InterestItem } from "./interest/interestUtils";
...
export function InterestPage() {
    ...
    const [viewsLimit, setViewsLimit] = useState(() => {
        ...
        if (saved) {
            try {
                const { limit, timestamp } = JSON.parse(saved) as Record<string, any>;
                if (limit && Date.now() - timestamp < SCROLL_RESTORATION_TIMEOUT_MS) return limit;
            } catch (e) {}
        }
    ...
    const activeItems = useMemo(() => (activeTab === "views" ? views : taps), [activeTab, taps, views]);
    ...
    const handleLoadMore = useCallback(() => {
        startTransition(() => {
            if (activeTab === "views") setViewsLimit((prev: number) => prev + ITEMS_PER_PAGE);
            else setTapsLimit((prev: number) => prev + ITEMS_PER_PAGE);
        });
    }, [activeTab]);
    ...
    useEffect(() => {
        ...
        if (maxViews > prevMaxViewsTs.current) {
            ...
            if (activeTab === "views") feedContainerRef.current?.scrollTo({ top: 0, behavior: "smooth" });
        }
        ...
        if (maxTaps > prevMaxTapsTs.current) {
            ...
            if (activeTab === "taps") feedContainerRef.current?.scrollTo({ top: 0, behavior: "smooth" });
        }
    ...
    useEffect(() => {
        ...
        if (!container) return;
        const handleScroll = () => {
            const scrollData = { top: container.scrollTop, limit: activeTab === "views" ? viewsLimit : tapsLimit, timestamp: Date.now() };
    ...
    useLayoutEffect(() => {
        if (displayedItems.length > 0 && !isLoading && !hasRestoredScroll && feedContainerRef.current) {
            ...
            if (saved) {
                try {
                    const { top, timestamp } = JSON.parse(saved) as Record<string, any>;
                    if (Date.now() - timestamp < SCROLL_RESTORATION_TIMEOUT_MS) feedContainerRef.current.scrollTop = top;
    ...
    useEffect(() => {
        if (data) markInterestSeen(Math.max(Date.now(), maxInterestTimestamp));
    ...
    useEffect(() => {
        ...
        if (ALWAYS_BOUNCE_FOR_DEBUG || !globalHasShownCount || demoMode !== 0) {
            const timer = setTimeout(() => { setShowCountLabel(true); globalHasShownCount = true; }, delay);
    ...
    const handleSetActiveTab = useCallback((nextTab: InterestTab) => {
    ...
    const handleOpenProfile = useCallback((profileId: string) => {
        navigate(`/profile/${profileId}`, { state: { returnTo: `${location.pathname}${location.search}` } });
    ...
    const handleTouchEnd = useCallback((event: TouchEvent<HTMLDivElement>) => {
        ...
        if (deltaX > 70 && activeTab === leftTab) handleSetActiveTab(rightTab);
        if (deltaX < -70 && activeTab === rightTab) handleSetActiveTab(leftTab);
                ...
                <div className="pointer-events-auto flex flex-col gap-3 mx-auto w-full max-w-4xl">
                    ...
                    <div className="flex flex-col gap-3">
                        <div className="flex h-12 items-center justify-between pl-[var(--app-px)] pr-6 -mt-1">
                            ...
                            >
                                <div className={cn("flex items-center transition-all duration-500", showCountLabel ? "justify-end" : "justify-center")}>
                                    <div className={cn("flex transition-all duration-500 ease-in-out overflow-hidden", showCountLabel ? "opacity-100 max-w-[200px] mr-0" : "opacity-0 max-w-0 mr-0 pointer-events-none")}>
                                    ...
                                    <div className="relative flex items-center justify-center">
                                        <div className={cn("flex items-center justify-center transition-all duration-500", activeTab === "views" ? "w-[51px]" : "w-12")}>
                                            <p className={cn("text-sm font-bold text-[var(--text-muted)] leading-none tabular-nums shrink-0 transition-opacity duration-300", (isFetching && !isQueryLoading) || isDemoLoading ? "opacity-0" : "opacity-100")}>
