...
export function SettingsPage() {
    ...
    const handleCheckUpdates = async () => {
        if (!isHotswapAvailable()) {
            toast.error(t("settings.ota_available_only_tauri", { defaultValue: "OTA updates are only available in the desktop app." }));
            return;
        }
        ...
        try {
            ...
            if (!result.available) {
                toast.success(t("settings.latest_version", { defaultValue: "You are on the latest version." }));
                return;
            }

            await installHotswapUpdate();
            toast.success(t("settings.update_installed", { defaultValue: "Update installed successfully!" }));
            window.location.reload();
        } catch (error) {
            const msg = getErrorMessage(error, t("settings.failed_update_check", { defaultValue: "Failed to check for updates." }));
    ...
    const handleSwitchUpdateChannel = async (channel: HotswapChannel) => {
        ...
        try {
            ...
            if (!result.requiresBinaryUpdate && result.available) {
                await installHotswapUpdate();
                toast.success(t("settings.switched_and_updated", { channel, defaultValue: `Switched to ${channel} and updated!` }));
                window.location.reload();
                return;
            }

            toast.success(t("settings.switched_channel", { channel, defaultValue: `Switched channel to ${channel}.` }));
            window.location.reload();
        } catch (error) {
            if (import.meta.env.DEV) {
                appLog.error("Switch update environment failed:", error);
            }
            toast.error(t("settings.failed_switch_env", { defaultValue: "Failed to switch update channel." }));
        } finally {
                        ...
                        <div className="flex items-start gap-3 px-4 py-3.5">
                            ...
                            <div className="min-w-0 flex-1">
                                <div className="grid grid-cols-[1fr_auto] gap-x-3">
                                    <p className="text-sm font-semibold leading-snug">{t("settings.check_updates", { defaultValue: "Check for updates" })}</p>
                                    <div className="row-span-2 flex items-start">
                                        {isSwitchingChannel ? (
                                            <span className="text-xs text-[var(--text-muted)]">{t("settings.switching", { defaultValue: "Switching..." })}</span>
                                        ) : isCheckingUpdates ? (
                                            <span className="text-xs text-[var(--text-muted)]">{t("settings.checking", { defaultValue: "Checking..." })}</span>
                                        ) : (
                                    ...
                                    <p className="mt-0.5 text-xs text-[var(--text-muted)]">{t("settings.check_updates_desc", { defaultValue: "Download the latest OTA features." })}</p>
                                </div>
