...
export function createChatService(
    ...
    sendWebsocketOrT: ((payload: unknown) => Promise<void>) | ((key: string, options?: any) => string),
    tOrUndefined?: (key: string, options?: any) => string
) {
    const t = typeof tOrUndefined === "function" ? tOrUndefined : (sendWebsocketOrT as (key: string, options?: any) => string);
    return {
        ...
        async listConversations(params?: {
            ...
            for (const entry of parsed.entries) {
                ...
                if (shouldBlock && window.localStorage.getItem("fg-autoblock-skip-after-two") === "true" && data.conversationId) {
                    try {
                        const log = await chatLog.readLog(data.conversationId);
                        let outgoingCount = 0;
