...
export function SelectableItem({ id, name, profileId, viewType, children, onNormalClick }: SelectableItemProps) {
    ...
    return (
        <div 
            role="button"
            className="relative group transition-transform duration-200 select-none cursor-pointer"
            onContextMenu={handleContextMenu}
            onClick={handleClick}
            style={{ WebkitTouchCallout: "none" }}
            {...(!isModeActiveForThisView ? longPressGestures : {})}
        >
            <div className={`transition-all duration-300 ${selected ? "scale-[0.96] opacity-70" : ""}`}>
                <div className={isModeActiveForThisView ? "pointer-events-none" : ""}>
                    {children}
                </div>
            </div>

            <div 
                className={`absolute inset-0 z-10 pointer-events-none rounded-xl border-[3px] transition-all duration-300 ${
