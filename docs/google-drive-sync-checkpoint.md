...
- `bunx tauri build --no-bundle` — passes after schema-v4 identity hardening and produces a fresh `src-tauri/target/release/free-grind.exe`. The first resumed attempt reached the final link but found that exact workspace executable running; after stopping only that verified process, the incremental retry completed successfully. This validation build intentionally has Drive unavailable because `FREE_GRIND_GOOGLE_DESKTOP_CLIENT_ID` is not configured in the current environment; release functionality still requires the real desktop OAuth client ID at compile time.
- `bun test` — 113 pass, 0 fail, 637 assertions across 12 files after schema-v4 identity hardening.
- `cargo test --lib` in `src-tauri` — 20 pass, 0 fail after native active-profile/client-ID regression coverage.
...
- Release workflow static checkpoint — Node's YAML parser accepts both workflow files and targeted whitespace checks pass. Both GitHub iOS paths require the iOS OAuth ID, regenerate/link AuthenticationServices, merge the callback plist, and build with `custom-protocol`; the Windows release path receives the desktop OAuth ID. `actionlint`, Xcode, `xcodegen`, and `sh` remain unavailable on this host.
...
- Neither OAuth client ID is configured in the current Windows shell. Both required GitHub repository secret names were verified through GitHub without revealing their values, so the feature-branch CI workflow can supply them to the corresponding builds. No client secret belongs in the app.
...
- **Statically resolved; platform validation remains:** both release workflows now require the appropriate OAuth ID and align iOS native authentication/Info.plist/custom-protocol steps. YAML parsing and whitespace checks pass on Windows; macOS CI must still prove generated-project/plugin compilation and IPA contents.
...
Do not repeat the completed audits, Windows matrix, OAuth provisioning, CI build, artifact inspection, backup verification, or Windows build switch. In the same Google Cloud project as the two OAuth clients, add the intended personal Google account under Google Auth Platform -> Audience -> Test users; the first authorization attempt was correctly formed but Google denied it because the app is still in Testing and the account was not approved. Retry Connect Google Drive in the currently running credential-enabled Windows build, complete authentication manually, then wait until the app reports Up to date and Last successful sync is no longer Never. Do not reveal a pairing code or install/pair the iPhone before that authoritative laptop upload succeeds. Then over-install the unsigned IPA through the same TrollStore entry, pair it, and perform key-persistence, token-refresh, laptop-off/phone-offline convergence, and force-quit launch/resume catch-up. Keep media policy forced off and keep manual backup/import untouched.

## OAuth activation checkpoint completed on 2026-09-02

- GitHub lists both repository secret names: `FREE_GRIND_GOOGLE_DESKTOP_CLIENT_ID` and `FREE_GRIND_GOOGLE_IOS_CLIENT_ID`. Their values were neither read nor printed.
