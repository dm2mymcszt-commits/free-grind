...
Do not repeat the completed audits, Windows matrix, OAuth provisioning, secret setup, artifact inspection, backup verification, build, or failed authorization retries. The corrected Windows artifact is already running from `dist/google-drive-test-ada05c21/free-grind-windows-x64/free-grind.exe` with `--child --instance=free-grind`, bound to the authoritative profile root. The user must visibly open Settings, click Connect Google Drive once, complete consent with the approved test account, and return to the app. Then observe the first authoritative upload until Windows reports Up to date and Last successful sync is no longer Never, followed by a real refresh test. Do not reveal a pairing code or install/pair the iPhone before those Windows checks pass. Keep media policy forced off and keep manual backup/import untouched.
